...
![](/Users/chenpoming/Desktop/mutate.png)
![](/Users/chenpoming/Desktop/mutate2.png)
...
```{r}
NBA1718 %>% filter(Team == "HOU") %>% mutate(avg_points = TotalPoints/GamesPlayed) %>% select(1:5, avg_points)
```
...
Hint: 這邊最一開始額外新增兩個標準，出賽場次>50場，每場至少出手一次。
...
```{r}
NBA1718 %>% filter(GamesPlayed >= 50, ThreesAttempted >= GamesPlayed) %>%
